...
void tvt_t::verify_permissions(insn_t insn, bool write) const {
  basic_csr_t::verify_permissions(insn, write);
  if (!proc->extension_enabled(EXT_SMCLIC) && !proc->extension_enabled(EXT_SSCLIC) && !proc->extension_enabled(EXT_SUCLIC))
    throw trap_illegal_instruction(insn.bits());
}
...
reg_t intstatus_t::read() const noexcept {
  ...
  if (address == CSR_SINTSTATUS) {
    ...
    val = val & mask;
  } else if (address == CSR_UINTSTATUS) {
